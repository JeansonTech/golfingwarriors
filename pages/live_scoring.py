...
st.markdown(
    ...
    @media (max-width: 768px) {
        ...
        /* ---------------------------------------------------
           COMPACT 9-HOLE NAVIGATION

           Streamlit normally allows columns to wrap on a
           narrow phone screen. That caused the 9 buttons to
           become 2-per-row and very wide. The hole navigation
           has its own keyed container so ONLY these columns
           are forced to remain nine-across.
           --------------------------------------------------- */

        .st-key-hole_nav div[data-testid="stHorizontalBlock"] {
        ...
        .st-key-hole_nav div[data-testid="stHorizontalBlock"] > div[data-testid="stColumn"] {
        ...
        .st-key-hole_nav div[data-testid="stButton"] {
            width: 100% !important;
            margin: 0 !important;
        }

        .st-key-hole_nav div[data-testid="stButton"] > button {
            width: 100% !important;
            min-width: 0 !important;
...
# ------------------------------------------------------------
# Tap-to-jump hole buttons
# ------------------------------------------------------------

st.caption("Tap a hole number to jump straight to it.")

with st.container(key="hole_nav"):

    for row_start in (1, 10):

        hole_cols = st.columns(9)

        for offset, hole_number in enumerate(
            range(row_start, row_start + 9)
        ):

            with hole_cols[offset]:

                if hole_number == current_hole:
                    button_label = f"⛳ {hole_number}"
                    button_type = "primary"
                elif hole_completed[hole_number]:
                    button_label = f"✓ {hole_number}"
                    button_type = "secondary"
                else:
                    button_label = str(hole_number)
                    button_type = "secondary"

                if st.button(
                    button_label,
                    key=f"jump_hole_{event_id}_{selected_group}_{hole_number}",
                    use_container_width=True,
                    type=button_type
                ):
                    st.session_state[current_hole_key] = hole_number
                    st.rerun()
...
# ============================================================
# MATCH PLAY LEADERBOARD HELPERS
# ============================================================

def get_match_play_matches_for_scoring(event_id):
    """Load Match Play setup for the current event."""
    connection = get_connection()
    try:
        return pd.read_sql_query(
            """
...
def build_match_play_results(match_df, score_lookup, holes):
    """Calculate live Match Play status from the saved hole scores.

    Handicaps are applied using the standard "drop to zero" approach:
    the lowest handicap in each match receives zero strokes and all other
    players receive the difference from that lowest handicap.

    Singles compare one player's net score against the other player.
    Teams compare the best net score of the two players on each side.
    """
    if match_df.empty:
        return []
    ...
    for match_number in sorted(match_df["match_number"].unique()):
        ...
        for side_number in (1, 2):
            side_rows = match_rows[
                match_rows["side_number"] == side_number
            ]
            for _, row in side_rows.iterrows():
                player_id = int(row["player_id"])
                if player_id in player_by_id:
                    side_players[side_number].append(player_by_id[player_id])

        all_match_players = side_players[1] + side_players[2]
        ...
        adjusted_handicaps = {
            int(player["player_id"]): max(
                0.0,
                float(player["event_handicap"]) - lowest_handicap
            )
            for player in all_match_players
        }
        ...
        for hole in hole_rows:
            ...
            # Only count a hole when every player in the match has a saved
            # score. This prevents a partially entered hole affecting the match.
            if any(
            ...
            for side_number in (1, 2):
                net_scores = []
                for player in side_players[side_number]:
                    gross = int(
                        score_lookup[(int(player["player_id"]), hole_number)]
                    )
                    net = calculate_net_score(
                        gross,
                        adjusted_handicaps[int(player["player_id"])],
                        stroke_index
                    )
                    net_scores.append(int(net))

                if net_scores:
                    # Betterball: the lowest net score wins the hole.
                    side_net_scores[side_number] = min(net_scores)

            if 1 not in side_net_scores or 2 not in side_net_scores:
                continue

            if side_net_scores[1] < side_net_scores[2]:
                hole_diffs.append(1)
            elif side_net_scores[2] < side_net_scores[1]:
                hole_diffs.append(-1)
            else:
                hole_diffs.append(0)

            completed_holes.append(hole_number)

        match_score = sum(hole_diffs)
        holes_played = len(completed_holes)
        ...
        if match_score == 0:
            status_text = "ALL SQUARE"
        else:
            status_text = f"{margin} UP" if leader_side == 1 else f"{margin} DN"

        # A match is won when the lead is greater than the holes remaining.
        ...
        def side_label(side_number):
            names = [player["name"] for player in side_players[side_number]]
...
def format_to_par(value):
    """Display a golf-style score-to-par value."""
    if value is None:
        return "—"
    value = int(value)
    if value == 0:
...
def calculate_live_to_par(result, score_type):
    """Calculate score-to-par for the holes the player has completed.

    IPS uses 2 points as the par-equivalent for each hole, so 3 IPS
    points is -1, 2 is E and 1 is +1. NET compares each hole's net
    score with the hole par.
    """
    player_id = int(result["player_id"])
    player_scores = score_dicts.get(player_id, {})
    total = 0

    for hole in holes_df.to_dict("records"):
        hole_number = int(hole["hole_number"])
        if hole_number not in player_scores:
            continue

        gross = int(player_scores[hole_number])
        par_value = int(hole["par"])
        stroke_index_value = int(hole["stroke_index"])
        ...
        if score_type == "IPS":
            ...
            total += net_score - par_value

    return total


def build_player_leaderboard(score_type):
    ...
    if score_type == "IPS":
        ...
        score_column = "IPS"
        score_values = lambda result: result["ips_total"]
    else:
        sorted_results = sorted(
            round_results,
            key=lambda result: (
        ...
        score_column = "Net"
        score_values = lambda result: result["net_total"]
        ...
        rows.append(
            {
                ...
                "To Par": format_to_par(
                    calculate_live_to_par(result, score_type)
                ),
                score_column: score_values(result)
            }
        )
...
# Load Match Play setup once for the leaderboard tabs.
match_play_df = get_match_play_matches_for_scoring(event_id)
match_play_results = build_match_play_results(
    match_play_df,
    score_lookup,
    holes_df.to_dict("records")
)
