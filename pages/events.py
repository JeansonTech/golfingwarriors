...
def ensure_event_player_status_column():
    """Add per-event player status for safe withdrawals/no-shows."""
    connection = get_connection()
...
# Ensure per-event player status exists before the page uses it.
try:
    ensure_event_player_status_column()
except Exception as error:
    st.error("Unable to initialise event player status storage.")
    st.exception(error)
    st.stop()
                ...
                if current_event_df.empty:
                    ...
                    if edit_format in ("MATCH PLAY TEAMS", "MATCH PLAY SINGLES"):
                        ...
                        for player_index, player in current_players_df.iterrows():
                            ...
                            pcol1, pcol2, pcol3, pcol4 = st.columns([3, 1, 1.5, 1.5])
                            ...
                            with pcol3:
                                edit_status = st.selectbox(
                                    ...
                                    format_func=lambda x: "Active" if x == "ACTIVE" else "Withdrawn / No-show",
                                    key=f"edit_status_{event_id}_{player_id}"
                                )

                            with pcol4:
                                edit_scorer = st.checkbox(
                                    "Scorer",
                                    value=bool(player["is_scorer"]) and edit_status == "ACTIVE",
                            ...
                            edit_event_players.append({
                                ...
                                "is_scorer": bool(edit_scorer) if edit_status == "ACTIVE" else False,
                                "status": edit_status
                            })
                        ...
                        for group_number, group_players in edit_groups.items():
                            ...
                            if active_players and scorer_count != 1:
                                edit_errors.append(
                                    f"Fourball {group_number} must have exactly ONE active scorer."
                                )
                            if len(group_players) > 4:
                                edit_errors.append(
                                    f"Fourball {group_number} has more than four players."
